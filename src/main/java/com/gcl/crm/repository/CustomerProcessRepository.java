...
public interface CustomerProcessRepository  extends JpaRepository<Customer,Integer> {
    @Query("SELECT new Customer (c.customerId, c.customerName, c.phoneNumber, c.email, c.number, c.contractNumber, c.source) FROM Customer c WHERE  c.contractNumber like %?1% order by c.createDate desc ")
    ...
    @Query("SELECT new Customer (c.customerId, c.customerName, c.phoneNumber, c.email, c.description) FROM Customer c WHERE c.customerName like %?1% and  c.phoneNumber like %?2% and c.email like %?3% and  c.number like %?4% order by  c.createDate desc ")
    List<Customer> searchWaitingCustomer(String name, String phone , String email,String number );
